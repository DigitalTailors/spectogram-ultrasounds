...
function callback(stream) {
    ...
    // PREPARE THE BROWSER BY SETTING THE DESIRED SAMPLIG RATE
    const desiredRate = 96000; // For example, 44.1 kHz, 96 kHz, 192 kHz, Max on Microsoft Edge could be 768 kHz
    const audioContext = new AudioContext({ sampleRate: desiredRate });
    console.log("Requested sample rate:", desiredRate);
    console.log("Actual sample rate:", audioContext.sampleRate);

    /*
    !!! YOU MAY GET THIS ERROR FROM THE BROWSER IF YOU PASS THE SAMPLING LIMIT
    NotSupportedError: Failed to construct 'AudioContext': The hardware sample rate provided (5.19200e+6) is outside the range [3000, 768000].
    at callback (spectrogram.js:114:26)
    at onSuccess (spectrogram.js:48:9)
    callback	@	spectrogram.js:114
    onSuccess	@	spectrogram.js:48
    Promise.then		
    (anonimo)	@	spectrogram.js:55
    */
...
function YaxisMarks() {
    ...
    if (document.getElementById("scale").value == "Linear") {
        var Yaxis = new Array;
        Yaxis = [
            100, 500, 1000, 1500, 2000, 2500, 3000, 3500, 4000, 4500, 5000, 5500, 6000, 6500, 
            7000, 7500, 8000, 8500, 9000, 9500, 10000, 11000, 12000, 13000, 14000, 15000, 16000, 
            ...
            110000, 120000, 130000, 140000, 150000, 160000, 170000, 180000, 190000, 200000, 210000, 
        ...
        var Yaxis = new Array;
        Yaxis = [
            100, 200, 400, 600, 800, 1000, 2000, 3000, 4000, 5000, 6000, 7000, 8000, 9000, 
            10000, 11000, 13000, 15000, 17000, 20000, 25000, 30000, 35000, 40000, 50000, 60000, 
            70000, 80000, 90000, 100000, 120000, 140000, 160000, 180000, 200000, 220000, 250000, 
